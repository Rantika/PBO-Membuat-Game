...
public class BuayaMenu implements Screen {
    private Stage uiStage;
    private Game game;

    public BuayaMenu(Game g)
    {
        game = g;
        create();
    }
    public void create()
    {
        ...
        background.setTexture( new
                Texture(Gdx.files.internal("tiles-menu.jpeg")) );
        uiStage.addActor( background );
        BaseActor titleText = new BaseActor();
        titleText.setTexture( new
                Texture(Gdx.files.internal("cheese-please.png")) );
...
// repeating color pulse effect
    ...
    public void render(float dt)
    {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.S))
            game.setScreen((Screen) new BuayaLevel(game));
            // update
        uiStage.act(dt);
            // draw graphics
